...
function refreshCart() { if ($('#cart').classList.contains('efm-on')) drawCart(); }
...
function drawCart() {
  ...
  if (booked) {
    body.innerHTML = `<div class="efm-done"><div class="efm-tick"><svg class="efm-ic"><use href="#check"/></svg></div>
      ...
      <div class="efm-c-box" style="text-align:left"><div class="efm-addr"><svg class="efm-ic"><use href="#pin"/></svg>
      <span>402, Lake View Apartments<br/>HSR Layout Sector 2, Bengaluru 560102</span></div></div></div>`;
    foot.innerHTML = `<button class="efm-pay" id="doneBtn"><span>Track booking</span><svg class="efm-ic"><use href="#chev-r"/></svg></button>`;
    return;
  }
  if (!count()) {
    body.innerHTML = `<div class="efm-c-empty"><svg class="efm-ic"><use href="#bag"/></svg>
    ...
    foot.innerHTML = `<button class="efm-pay" id="shopBtn" style="background:var(--bg-2); color:var(--ink); border:1px solid var(--line-2)">
      <span>Browse services</span><svg class="efm-ic"><use href="#chev-r"/></svg></button>`;
    return;
  }
  const b = bill(), saved = Math.round(mrpTotal() - subtotal()) + b.disc;
  body.innerHTML = `
    <div class="efm-c-box"><b>${count()} item${count() > 1 ? 's' : ''} · ${units()} unit${units() > 1 ? 's' : ''}</b>
    ${Object.entries(cart).map(([key, l]) => {
    const s = byId[l.id], p = priceOf(l.id, l.t, l.u);
    return `<div class="efm-c-item">
        <span class="efm-th" style="background:${P[s[8]]}"><svg class="efm-ic" style="color:${C[s[8]]}"><use href="#${s[7]}"/></svg></span>
        <span class="efm-nm"><b>${s[1]}</b>
          <button class="efm-vr" data-edit="${key}"><svg class="efm-ic"><use href="#edit"/></svg>${typeOf(l.id, l.t)[1]} · ${unitOf(l.id, l.u)[1]}</button>
          <div style="font-size:10.5px; color:var(--gray); margin-top:3px">${rupee(p)} × ${qtyText(l.id, l.u, l.q)}</div>
        </span>
        <span class="efm-rt">
          <span class="efm-step">
            <button data-k-dec="${key}" aria-label="Decrease"><svg class="efm-ic"><use href="#minus"/></svg></button>
            <span>${l.q}</span>
            <button data-k-inc="${key}" aria-label="Increase"><svg class="efm-ic"><use href="#plus"/></svg></button>
          </span>
          <span class="efm-pp">${rupee(p * l.q)}</span>
        </span></div>`;
  }).join('')}</div>

    <div class="efm-c-box"><b>Slot</b>
      <div class="efm-chips">${DATES.map(d => `<button class="efm-chip ${d.k === dateSel ? 'efm-on' : ''}" data-date="${d.k}">${d.top}<small>${d.num}</small></button>`).join('')}</div>
      <div class="efm-slot-grid">${SLOTS.map((s, i) => `<button class="efm-chip ${s === slotSel ? 'efm-on' : ''}" data-slot="${s}" ${i ? '' : 'disabled'}>${s}</button>`).join('')}</div>
    </div>

    <div class="efm-c-box"><b>Address</b>
      <div class="efm-addr"><svg class="efm-ic"><use href="#pin"/></svg>
        <span>402, Lake View Apartments<br/>HSR Layout Sector 2, Bengaluru 560102</span>
        <button class="efm-see" style="margin-left:auto">Change</button></div></div>

    <div class="efm-c-box"><b>Bill</b>
      <div class="efm-bill">
        ...
        <div class="efm-tot"><span>To pay after service</span><b>${rupee(b.total)}</b></div>
      </div>
  ...
  foot.innerHTML = `<button class="efm-pay" id="confirm">
    <span>${rupee(b.total)} <small style="font-weight:500; opacity:.85">· pay after service</small></span>
    <span class="efm-row" style="gap:6px">Confirm booking <svg class="efm-ic"><use href="#chev-r"/></svg></span></button>`;
}

export function openCart() { booked = false; drawCart(); $('#cart').classList.add('efm-on'); $('#scrim').classList.add('efm-on'); document.body.style.overflow = 'hidden'; syncCards(); }
export function closeCart() { $('#cart').classList.remove('efm-on'); $('#scrim').classList.remove('efm-on'); document.body.style.overflow = ''; syncCards(); }
